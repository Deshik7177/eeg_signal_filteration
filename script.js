...
class SignalEngine {
    ...
    async cycle() {
        ...
        const noisy = base.map(v => v + (Math.random() - 0.5) * CONFIG.NOISE_LEVEL * this.noiseMultiplier);
        ...
        const snr = this.calculateSNR(base, noisy);
...
class UIManager {
    ...
    render(data) {
        document.getElementById('val-snr').textContent = data.snr.toFixed(1);
        const noiseRatio = Math.pow(10, -data.snr / 10);
